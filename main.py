...
def register():
    ...
    if form.validate_on_submit():
        query_user = db.session.query(User).filter_by(email=form.email.data).first()
        if query_user:
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
    return render_template("post.html", post=requested_post, logged_in=current_user.is_authenticated, current_user=current_user, form=form, year=year)
